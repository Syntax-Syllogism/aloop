...
aloop is in pre-1.0 development. Only the current release receives
security fixes.
...
- The aloop source in this repository.
...
- Issues that require an attacker to already have local code-execution
  privileges equivalent to the aloop process (we assume the local
  user is trusted).
